<template>
    <app-layout title="Inventory">
                    ...
                    <div class="card shadow bg-light">
                        <div class="card-body bg-white px-5 py-3 border-bottom rounded-top">
                            <DataTable lazy responsiveLayout="scroll" paginator
                                ...
                                filterDisplay="menu"
                                sortMode="multiple" :value="orders" :rows="10" :rowsPerPageOptions="[10, 20, 50]"
        ...
        <Dialog v-model:visible="show_selected_order" modal :breakpoints="{ '2000px': '75vw', '640px': '100vw' }">
            <template #header>
            ...
            <div class="row" v-for="(product, index) in selected_order.products" :key="`order-product-${index}`">
                <div class="col">
        ...
        <Dialog v-model:visible="new_order_form_visible" :modal="true"
            ...
            <form @submit.prevent="submit_order">
                <div class="row">
                    <div class="col">
                        <InputNumber v-model="new_order.total" mode="currency" currency="PHP" locale="en-US"
                            placeholder="Total"
                            :disabled="submitting" required />
                    </div>
                    <div class="col">
                        <Calendar v-model="new_order.created_at" :showTime="true" class="w-100" placeholder="Order Date"
                            :disabled="submitting" required />
                    </div>
                </div>
                <div class="row mt-4 mb-2">
                    ...
                    <div class="col-auto">
                        <Button @click="increment_products" class="me-2">+ Add Product</Button>
                    </div>
                </div>
                <div v-for="(product, index) in new_order.products" class="row mt-2" :key="index">
                    <div class="col-1">
                        <Button @click="remove(index)" v-if="index !== 0">x</Button>
                    </div>
                    <div class="col-4">
                        <Dropdown v-model="product.id" :options="products" option-label="name" option-value="id"
                            placeholder="Select a Product" filter class="w-100" />
                    </div>
                    <div class="col-3">
                        <InputNumber v-model="product.quantity" @input="update_to_use($event.value, index)"
                            placeholder="Quantity" class="w-100" />
                    </div>
                    <div class="col-3">
                        <InputNumber v-model="product.to_use" class="w-100" placeholder="Stock Used" />
                    </div>
                </div>
                <div class="row mt-4">
    ...
    methods: {
        async submit_order() {
            try {
                ...
                this.new_order_form_visible = false;
                this.search();
            } catch (e) {
                console.log(e);
            }
