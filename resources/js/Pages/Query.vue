                ...
                <div class="col-md-12">
                    <div class="card shadow bg-light">
                        <div class="card-body bg-white px-5 py-3 border-bottom rounded-top">
                            ...
                            <div class="row mb-4" v-for="(selected, index) in selection" :key="`product-${index}`">
                                <div class="col-1">
                                    <Button @click="remove(index)" v-if="index !== 0">x</Button>
                                </div>
                                <div class="col-5">
                                    <Dropdown v-model="selected.product" :options="selected.products"
                                        option-label="name"
                                        option-value="id" placeholder="Select a Product" filter
                                        @filter="filter_products($event, index)" :loading="selected.loading"
                                        class="w-100" />
                                </div>
                                <div class="col-3">
                            ...
                            <h5 class="card-title mt-5">
                                You lack:
                                <ProgressSpinner v-if="query_loading" style="width: 25px; height: 25px" />
                            </h5>
                            ...
                            <div class="row">
                                <div class="card col-auto" style="width: 18rem" v-for="resource in resources"
                                    ...
                                    <div class="card-body">
                                        ...
                                        <h5 class="card-subtitle" :class="{ 'text-success': resource.quantity === 0 }">
                                            {{ resource.resource.name }}
                                            <svg v-if="resource.quantity === 0" xmlns="http://www.w3.org/2000/svg"
                                                width="16"
                                        ...
                                        <p class="card-text">
                                            You need <strong>{{ resource.required }}</strong> {{ resource.resource.unit
                                                ||
                                                "Units" }}. <br />
                                            You have
                                            <strong>{{ resource.resource.stock }}</strong>
                                            {{ resource.resource.unit || "Units" }}.<br />
                                            You need <strong>{{
                                                resource.quantity }}</strong> more.
                                        </p>
                                    </div>
                                </div>
                            </div>
